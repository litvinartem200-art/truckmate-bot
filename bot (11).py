...
# ─── API: СТОЯНКИ TIR (Overpass — расширенный поиск) ─────────────────────────
async def find_tir_parkings_near(lat: float, lon: float, radius_km: int = 50) -> list:
    """
    Ищет стоянки для грузовиков через Overpass API.
    Расширенный запрос — находит гораздо больше объектов чем раньше.
    Включает: truck_stop, parking с любым hgv тегом, rest_area, layby.
    """
    try:
        r_m = radius_km * 1000
        # Расширенный запрос — все варианты тегов которые используют водители в OSM
        query = f"""
[out:json][timeout:25];
(
  node["amenity"="truck_stop"](around:{r_m},{lat},{lon});
  way["amenity"="truck_stop"](around:{r_m},{lat},{lon});
  relation["amenity"="truck_stop"](around:{r_m},{lat},{lon});

  node["amenity"="parking"]["hgv"](around:{r_m},{lat},{lon});
  way["amenity"="parking"]["hgv"](around:{r_m},{lat},{lon});

  node["amenity"="parking"]["truck"="yes"](around:{r_m},{lat},{lon});
  way["amenity"="parking"]["truck"="yes"](around:{r_m},{lat},{lon});

  node["amenity"="rest_area"]["hgv"](around:{r_m},{lat},{lon});
  way["amenity"="rest_area"]["hgv"](around:{r_m},{lat},{lon});
  node["amenity"="rest_area"]["truck"="yes"](around:{r_m},{lat},{lon});
  way["amenity"="rest_area"]["truck"="yes"](around:{r_m},{lat},{lon});

  node["highway"="rest_area"]["hgv"](around:{r_m},{lat},{lon});
  way["highway"="rest_area"]["hgv"](around:{r_m},{lat},{lon});

  node["amenity"="parking"]["capacity:hgv"](around:{r_m},{lat},{lon});
  way["amenity"="parking"]["capacity:hgv"](around:{r_m},{lat},{lon});
);
out center 10;
"""
        async with aiohttp.ClientSession() as s:
            async with s.post(
                ...
                if r.status == 200:
                    ...
                    # Сортируем по расстоянию от точки
                    def dist_to_point(p):
                        plat = p.get("lat") or p.get("center",{}).get("lat", lat)
                        plon = p.get("lon") or p.get("center",{}).get("lon", lon)
                        if plat and plon:
                            return haversine(lat, lon, plat, plon)
                        return 999
                    elements.sort(key=dist_to_point)
                    return elements
    except Exception as e:
...
def google_maps_parking_search(lat: float, lon: float, lang: str) -> str:
    """Ссылка на поиск парковок для грузовиков в Google Maps"""
    query = "truck+parking+TIR" if lang == "en" else \
            "parking+camion+poids+lourd" if lang == "fr" else \
            "стоянка+для+фур+TIR"
    return f"https://www.google.com/maps/search/{query}/@{lat},{lon},13z"
...
def format_parking_short(p: dict, ref_lat: float, ref_lon: float) -> str:
    ...
    # Название — пробуем все возможные теги
    name = (tags.get("name")
    ...
    if tags.get("restaurant")=="yes" or tags.get("food")=="yes" or tags.get("cafe")=="yes":
    ...
    if tags.get("fuel")=="yes" or tags.get("fuel:diesel")=="yes": amenities.append("дизель")
    cap_hgv = tags.get("capacity:hgv","")
    if cap_hgv and cap_hgv.isdigit():              amenities.append(f"{cap_hgv} мест")

    fee_tag = tags.get("fee","")
    fee = " (плат.)" if fee_tag not in (None,"","no") else " (беспл.)" if fee_tag=="no" else ""
    am = " · ".join(amenities) if amenities else ""
    maps_url = f"https://maps.google.com/?q={clat},{clon}"
    navi_url = f"https://maps.google.com/maps?daddr={clat},{clon}&dirflg=d"
...
# ─── ГЛАВНАЯ ФУНКЦИЯ МАРШРУТА ─────────────────────────────────────────────────
async def build_full_route_message(city_from: str, city_to: str,
                                    weight: float, lang: str) -> list:
    """
    Возвращает СПИСОК строк — каждая отправляется отдельным сообщением.
    Это обходит лимит Telegram 4096 символов.

    Сообщение 1: маршрут + стоимость + навигация + пробки + погода
    Сообщение 2: стоянки TIR (только если найдены)
    """
    import asyncio
    t = T[lang]

    # 1. Геокодинг обеих точек
    lon1, lat1, _, cc1 = await geocode(city_from)
    lon2, lat2, _, cc2 = await geocode(city_to)
    if not (lon1 and lon2):
        return [t["not_found"]]

    # 2. Маршрут — ORS если есть ключ, иначе OSRM
    waypoints = []
    rtype = "STD"
    km, dur = None, None
    ...
    if not km:
        ...
        if not km:
            return [t["not_found"]]
        waypoints = [(lat1, lon1)] + interpolate_points(lat1, lon1, lat2, lon2, 200) + [(lat2, lon2)]

    if not waypoints:
        waypoints = interpolate_points(lat1, lon1, lat2, lon2, 200)

    # 3. Контрольные точки: начало + середина + конец (максимум 3)
    if len(waypoints) >= 3:
        mid = len(waypoints) // 2
        pts = [waypoints[0], waypoints[mid], waypoints[-1]]
    elif len(waypoints) == 2:
        pts = [waypoints[0], waypoints[-1]]
    else:
        pts = [(lat1, lon1), (lat2, lon2)]

    # 4. Параллельно запрашиваем: пробки + погода + стоянки
    async def fetch_all(pt_lat, pt_lon):
        results = await asyncio.gather(
            get_tomtom_traffic(pt_lat, pt_lon),
            get_tomtom_incidents(pt_lat, pt_lon),
            get_weather_point(pt_lat, pt_lon),
            find_tir_parkings_near(pt_lat, pt_lon, radius_km=50),
            return_exceptions=True
        )
        flow     = results[0] if not isinstance(results[0], Exception) else {}
        ...
        parks    = results[3] if not isinstance(results[3], Exception) else []
        return flow, incidents, weather, parks

    point_results = await asyncio.gather(*[fetch_all(p[0], p[1]) for p in pts])

    # 5. Собираем пробки, погоду, стоянки из всех точек
    traffic_lines = []
    weather_lines = []
    all_parkings  = []
    seen_park_ids = set()

    for flow, incidents, weather, parks in point_results:
        ft = parse_tomtom_flow(flow, lang)
        if ft and ft not in traffic_lines:
            traffic_lines.append(ft)
        for inc in parse_tomtom_incidents(incidents, lang):
            if inc not in traffic_lines:
                traffic_lines.append(inc)
        wa = check_weather_alert(weather, lang)
        if wa:
            for line in wa.split("\n"):
                if line and line not in weather_lines:
        ...
        for p in parks:
            ...
            if key not in seen_park_ids:
                seen_park_ids.add(key)
                all_parkings.append(p)

    # 6. Стоимость маршрута
    cost_text, _ = build_cost(km, cc1 or "FR", cc2 or "FR", weight, lang, rtype)
    cost_text = cost_text.replace("{DUR}", str(dur))

    # 7. Ссылки навигации
    gmaps = f"https://www.google.com/maps/dir/{lat1},{lon1}/{lat2},{lon2}"
    waze  = f"https://waze.com/ul?ll={lat2},{lon2}&navigate=yes"
    ...
    # ── СООБЩЕНИЕ 1: маршрут ──────────────────────────────────────────────────
    msg1_lines = [
        f"*{city_from.title()} — {city_to.title()}*",
        "",
        cost_text,
        "",
        f"*{nav_lbl}:*",
    ...
    msg1_lines.append(f"\n*{t['lbl_traffic']}:*")
    if traffic_lines:
        for tl in traffic_lines[:4]:
            msg1_lines.append(f"• {tl}")
    elif TOMTOM_KEY:
        msg1_lines.append(f"• {t['lbl_no_traffic']}")
    else:
        msg1_lines.append(f"• {t['lbl_no_tomtom']}")

    # Погода (только если есть предупреждения)
    if weather_lines:
        msg1_lines.append(f"\n*{t['lbl_weather']}:*")
        for wl in weather_lines[:3]:
            msg1_lines.append(f"• {wl}")
    ...
    if all_parkings:
        p2_lines = [f"*{t['lbl_parkings']}:*\n"]
        for i, p in enumerate(all_parkings[:5], 1):
            plat = p.get("lat") or p.get("center",{}).get("lat", mid_lat)
            plon = p.get("lon") or p.get("center",{}).get("lon", mid_lon)
            p2_lines.append(f"*{i}.* {format_parking_short(p, plat, plon)}")
        msg2 = "\n\n".join(p2_lines)
    else:
        # Ничего не нашли — даём ссылки на Google Maps
        q  = google_maps_parking_search(mid_lat, mid_lon, lang)
        q1 = google_maps_parking_search(lat1, lon1, lang)
        q2 = google_maps_parking_search(lat2, lon2, lang)
        no_titles = {
            "ru": ("Стоянки TIR не найдены в OSM.\nПоищи через Google Maps:",
                   "У отправления", "Середина маршрута", "У назначения"),
            "uk": ("Стоянок TIR не знайдено в OSM.\nПошукай через Google Maps:",
                   "Біля відправлення", "Середина маршруту", "Біля призначення"),
            "fr": ("Aucun parking TIR dans OSM.\nRechercher sur Google Maps:",
                   "Près du départ", "Mi-parcours", "Près de l'arrivée"),
            "en": ("No TIR parkings in OSM.\nSearch on Google Maps:",
                   "Near departure", "Mid-route", "Near destination"),
        }
        titles = no_titles.get(lang, no_titles["ru"])
        msg2 = (f"*{t['lbl_parkings']}:*\n\n"
                f"{titles[0]}\n\n"
                f"{titles[1]}: {q1}\n"
                f"{titles[2]}: {q}\n"
                f"{titles[3]}: {q2}")
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("lang_"):
        ...
        # Получаем список сообщений [маршрут, стоянки]
        messages = await build_full_route_message(city_from, city_to, weight, l)

        if not messages:
            await q.edit_message_text(t["not_found"], reply_markup=kb_back(uid), parse_mode="Markdown")
            return

        # Если функция вернула строку (ошибка) а не список
        if isinstance(messages, str):
            await q.edit_message_text(messages, reply_markup=kb_back(uid), parse_mode="Markdown")
            return

        # Первое сообщение — редактируем существующее (маршрут + пробки + погода)
        await q.edit_message_text(
            messages[0],
        ...
        # Остальные сообщения — отправляем новыми (стоянки)
...
async def message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "from":
        ...
        if not lon:
            ...
            return
        # Пробуем радиус 60 км, если пусто — расширяем до 100 км
        parkings = await find_tir_parkings_near(lat, lon, radius_km=60)
        if not parkings:
            parkings = await find_tir_parkings_near(lat, lon, radius_km=100)
        if not parkings:
            # Ничего не нашли — даём Google Maps поиск
            gmaps = google_maps_parking_search(lat, lon, l)
            no_osm = {
                "ru":f"В OpenStreetMap стоянок TIR не найдено в радиусе 100 км.\n\nПоищи через Google Maps:\n{gmaps}",
                "fr":f"Aucun parking TIR dans OSM dans 100 km.\n\nRechercher sur Google Maps:\n{gmaps}",
                "en":f"No TIR parkings in OSM within 100 km.\n\nSearch on Google Maps:\n{gmaps}",
                "uk":f"Стоянок TIR в OSM не знайдено в 100 км.\n\nПошукай через Google Maps:\n{gmaps}",
            }
            await update.message.reply_text(
                no_osm.get(l, no_osm["ru"]),
                reply_markup=kb_back(uid),
                parse_mode="Markdown",
                disable_web_page_preview=True)
            return
        lines = []
        for i, p in enumerate(parkings[:6], 1):
...
async def location_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if u.get("step") == "parking":
        ...
        if not parkings:
            parkings = await find_tir_parkings_near(loc.latitude, loc.longitude, radius_km=100)
        if not parkings:
            gmaps = google_maps_parking_search(loc.latitude, loc.longitude, l)
            no_osm = {
                "ru":f"В OSM стоянок не найдено.\nПоищи через Google Maps:\n{gmaps}",
                "fr":f"Aucun parking dans OSM.\nRechercher:\n{gmaps}",
                "en":f"No parkings in OSM.\nSearch:\n{gmaps}",
                "uk":f"Стоянок в OSM не знайдено.\nПошукай:\n{gmaps}",
            }
            await update.message.reply_text(
                no_osm.get(l, no_osm["ru"]),
                reply_markup=kb_back(uid),
